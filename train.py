...
# def evaluate(data, model, args, use_beam=True):
#     """在data上用训练好的模型进行预测"""
#     # trg = []
#     # res = []
#     bleu_ = 0
...
#         model.eval()
#         # 开始预测
#         test_loss = eval_epoch(data, model_par,
#                               MultiGPULossCompute(model.generator, criterion, args, None))
#         bleu_score = evaluate(data, model, args)
#         xprint.sprint('Test loss: {:.6f},  Bleu Score: {}'.format(test_loss, bleu_score))


# def ids2word(dict, ids, pad):
#     if type(ids) is int:
#         return dict.get(ids)
#     else:
#         result = []
